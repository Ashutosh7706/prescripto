...
<input onChange={(e)=>setImage(e.target.files[0])}type="file" id="image" hidden/>
      ...
      {
        isEdit ? <input className='bg-color-gray-50 text-3xl font-medium max-w-60 mt-4' type="text" value={userData.name} onChange={e => setUserData(prev => ({ ...prev, name: e.target.value }))} />
          : <p className='font-medium text-3xl text-neutral-800 mt-4'>{userData.name}</p>
      }
      <hr className='bg-zinc-400 h-[1px] border-none '/>
      <div>
        <p className='text-neutral-500 underline mt-3'>CONTACT INFORMATION</p>
        <div className='grid grid-cols-[1fr_2fr] gap-y-2.5 mt-3 text-neutralc'>
          ...
          <p className='font-medium'>Address:</p>
          {
            isEdit ? <p>
              ...
              <br />
              <input className='by-gray-50' onChange={(e) => setUserData(prev => ({ ...prev, address: { ...prev.address, line2: e.target.value } }))} value={userData.address.line2} type="text" />
            </p> :
